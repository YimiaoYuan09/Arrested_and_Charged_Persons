...
The analysis data was cleaned to exclude the "unknown" information on the sex and age_cohort column and "No Specified Address (NSA)" on the division column to obtain more specific and detailed data. The cleaned data will be analyzed and performed using R (@citeR) with `tidyverse` (@citeTidyverse), `knitr` (@citeKnitr), `dplyr` (@citeDplyr), `ggplot2` (@citeGgplot2) and `here` (@citeHere).
...
# a table of total number of arrested and charged persons in each year by division
...
ggplot(count_arrested_person_div, aes(x = division, y = count_arrest_div, 
                                      group = arrest_year, fill = as.factor(arrest_year))) +
  geom_bar(stat = "identity") +
  theme_minimal() +
